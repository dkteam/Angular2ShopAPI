...
export class CarLineComponent implements OnInit {
  ...
  loadData() {
    this._dataService.get('/api/carLine/getlistpaging?page=' + this.pageIndex + '&pageSize=' + this.pageSize + '&filter=' + this.filter)
      .subscribe((respone: any) => {
        this.carLines = respone.Items;
  ...
  loadDetail(id: any) {
    this._dataService.get('/api/carLine/detail/' + id)
      .subscribe((respone: any) => {
        this.entity = respone;
      });
  }
  ...
  saveChanged(form: NgForm) {
    if (form.valid) {
      if (this.entity.ID == undefined) {
        this._dataService.post('/api/carLine/add', JSON.stringify(this.entity))
          .subscribe((respone: any) => {
            this.loadData();
        ...
        this._dataService.put('/api/carLine/update', JSON.stringify(this.entity))
          .subscribe((respone: any) => {
            this.loadData();
            this.modalAddEdit.hide();
            form.resetForm();
  ...
  deleteItemConfirm(id: any) {
    this._dataService.del('/api/carLine/delete', 'id', id)
  ...
  public deleteMulti() {
    this.checkedItems = this.carLines.filter(x => x.Checked);
    var checkedIds = [];
    for (var i = 0; i < this.checkedItems.length; ++i)
      checkedIds.push(this.checkedItems[i]["ID"]);

    this._notificationService.printConfirmationDialog(MessageConstants.CONFIRM_DELETE_MSG, () => {
      this._dataService.del('/api/carLine/deletemulti', 'checkedCarLines', JSON.stringify(checkedIds)).subscribe((response: any) => {
        this._notificationService.printSuccessMessage(MessageConstants.DELETED_OK_MSG);
        this.loadData();
      }, error => this._dataService.handleError(error));
    });
  ...
  public enableDeleteButton() {    
    this.carLines.filter(x => x.Checked).length != 0 ? this.deleteButtonFlag = false : this.deleteButtonFlag = true;
  }
