...
@Component({
  ...
  styleUrls: ['./price-request.component.css']
})
export class PriceRequestComponent implements OnInit {
  ...
  loadData() {
    this._dataService.get('/api/priceRequest/getlistpaging?page=' + this.pageIndex + '&pageSize=' + this.pageSize + '&filter=' + this.filter)
      .subscribe((respone: any) => {
        this.priceRequests = respone.Items;
  ...
  deleteItemConfirm(id: any) {
    this._dataService.del('/api/priceRequest/delete', 'id', id)
  ...
  public deleteMulti() {
    this.checkedItems = this.priceRequests.filter(x => x.Checked);
    var checkedIds = [];
    for (var i = 0; i < this.checkedItems.length; ++i)
      checkedIds.push(this.checkedItems[i]["ID"]);

    this._notificationService.printConfirmationDialog(MessageConstants.CONFIRM_DELETE_MSG, () => {
      this._dataService.del('/api/priceRequest/deletemulti', 'checkedItems', JSON.stringify(checkedIds)).subscribe((response: any) => {
        this._notificationService.printSuccessMessage(MessageConstants.DELETED_OK_MSG);
        this.loadData();
      }, error => this._dataService.handleError(error));
    });
