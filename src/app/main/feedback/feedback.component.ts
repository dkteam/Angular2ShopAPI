...
export class FeedbackComponent implements OnInit {
  ...
  loadFeedback(id: any) {
    this._dataService.get('/api/feedback/detail/' + id)
      .subscribe((respone: any) => {
        this.entity = respone;
      });
  }
  ...
  saveChanged(valid: boolean) {
    if (valid) {
      if (this.entity.ID == undefined) {
        this._dataService.post('/api/feedback/add', JSON.stringify(this.entity))
          .subscribe((respone: any) => {
            this.loadData();
        ...
        this._dataService.put('/api/feedback/update', JSON.stringify(this.entity))
          .subscribe((respone: any) => {
            this.loadData();
            this.modalAddEdit.hide();
  ...
  deleteItemConfirm(id: any) {
    this._dataService.del('/api/feedback/delete', 'id', id)
  ...
  public deleteMulti() {
    this.checkedItems = this.feedbacks.filter(x => x.Checked);
    var checkedIds = [];
    for (var i = 0; i < this.checkedItems.length; ++i)
      checkedIds.push(this.checkedItems[i]["ID"]);

    this._notificationService.printConfirmationDialog(MessageConstants.CONFIRM_DELETE_MSG, () => {
      this._dataService.del('/api/feedback/deletemulti', 'checkedFeedbacks', JSON.stringify(checkedIds)).subscribe((response: any) => {
        this._notificationService.printSuccessMessage(MessageConstants.DELETED_OK_MSG);
        this.loadData();
      }, error => this._dataService.handleError(error));
    });
  ...
  public enableDeleteButton() {    
    this.feedbacks.filter(x => x.Checked).length != 0 ? this.deleteButtonFlag = false : this.deleteButtonFlag = true;
  }
